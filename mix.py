...
class Server:
    ...
    def handle_connection(self, connection):
        self.symmetric_key = self.encrypting_verify_and_get_symmetric_key(
            connection)

        try:
            self.connection = connection
            while self.handle_request(connection, self.symmetric_key):
                pass
        except Exception:
    ...
    def symmetric_decrypt(self, key, ciphertext):
        algorithm = algorithms.ARC4(key)
        cipher = Cipher(algorithm, mode=None)
    ...
    def encrypting_verify_and_get_symmetric_key(self, connection):
        ...
        encrypted_symmetric_key = connection.recv(2048)
        symmetric_key = self.decrypt_with_private_key(
            private_key, encrypted_symmetric_key)
        return symmetric_key

    def handle_request(self, connection, symmetric_key):
        self.curuser = {}
        ...
        request = self.symmetric_decrypt(symmetric_key, request)
        print(request)
                ...
                if file_path.is_file():
                    ...
                    if self.get_request_header('Range'):
                        ...
                        range_list = [item.split(
                            '-') if '-' in item else [item, ''] for item in range_list_raw]
                        for i in range_list:
                            if i[1] == '':
                                i[0] = int(i[0])
                            else:
                    ...
                    # 正常传输
                        ...
                        # 断点传输 单个区域
                            ...
                            self.create_response_header(
                                'Content-Range', f'bytes {range_temp[0]}-{right_temp}/{content_size}')
                            self.create_response_header(
                                'Content-Type', content_type)
                            self.create_response_header(
                                'Content-Length', right_temp - range_temp[0] + 1)
                            self.end_response_line()
                            self.end_response_headers()
                            if not isHead:
                                self.send_file_by_range(
                                    file_path, range_temp[0], range_temp[1])
                                self.end_response_payload()
                        elif len(range_list) > 1:
                            self.create_response_line(206, 'Partial Content')
                            self.create_response_header(
                                'Content-Type', f'multipart/byteranges; boundary={MYBOUNDARY}')
                            # 计算文件长度
                            content_length = 0
                            for i in range_list:
                            ...
                            for j in range(len(range_list)):
                                ...
                                #   在表头加boundary
                                if j == 0:
                                    self.send_multi_file_by_range(
                                        file_path, content_type, content_size, left_range_temp, right_range_temp, True)
                                else:
                                    self.send_multi_file_by_range(
                                        file_path, content_type, content_size, left_range_temp, right_range_temp, False)

                            self.create_response_header(
                                'Content-Length', len(self.response_payload))
                            self.end_response_line()
                            self.end_response_headers()
                            if not isHead:
                                self.end_response_payload()
    ...
    def send_multi_file_by_range(self, filepath, content_type, content_size, left_range, right_range, add_boundary_at_beginning):
        ...
        payload_to_write = payload_to_write + \
            f'Content-Range: bytes {left_range}-{right_range}/{content_size}\r\n\r\n'
    ...
    def end_response_headers(self):
        if self.request_line_sended:
            self.response_headers = self.symmetric_encrypt(
                self.symmetric_key, self.response_headers + (f"\r\n"))
            self.flush_headers()
            self.response_headers = f''
        else:
            print(
    ...
    def flush_headers(self):
        self.connection.sendall(self.response_headers)

    # encode and send response_line
    def end_response_line(self):
        self.connection.sendall(self.symmetric_encrypt(
            self.symmetric_key, self.response_line))
    ...
    def end_response_payload(self):
        self.connection.sendall(self.symmetric_encrypt(
            self.symmetric_key, self.response_payload))
        self.response_payload = b''
